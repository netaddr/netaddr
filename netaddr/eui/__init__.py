...
# Lazy import of `netaddr.eui.ieee`.
_ieee = None


def _maybe_import_netaddr_eui_ieee():
    """Lazy loading for IEEE data strictures.

    This method imports `netaddr.eui.ieee` if it has not been imported already,
    and stores the reference in the `_ieee` global. This prevents a superfluous
    import for every OUI or IAB object that is created.

    :returns: The `netaddr.eui.ieee` module.
    """
    global _ieee
    if _ieee is None:
